...
export default function NewsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#18191c] text-white flex flex-col font-sans selection:bg-[#e81f76] selection:text-white">
      ...
      {activeModalArticle && (
        <div
          className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 backdrop-blur-md p-3 sm:p-4"
          onClick={() => setActiveModalArticle(null)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative w-full max-w-2xl bg-[#232328] border-2 border-pink-500 rounded-3xl p-5 sm:p-8 text-white shadow-2xl overflow-y-auto max-h-[90vh]"
          >
            {/* Close Button */}
            <button
              onClick={() => setActiveModalArticle(null)}
              aria-label="Close Article"
              className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-[#e81f76] text-white h-9 w-9 sm:h-10 sm:w-10 rounded-full flex items-center justify-center font-bold text-lg sm:text-xl hover:scale-110 active:scale-95 transition cursor-pointer z-10 border-none outline-none"
            >
              ✕
            </button>

            <div className="relative aspect-[16/9] w-full rounded-2xl overflow-hidden mb-5 sm:mb-6 border-2 border-cyan-400">
              <Image
            ...
            <h2 className="text-2xl sm:text-4xl font-black text-[#e81f76] uppercase tracking-tight mt-1 mb-3 sm:mb-4">
            ...
            {/* Pembuka Modal Browser Popup */}
            <button
              onClick={() => {
                setActiveModalArticle(null);
                setIsGameModalOpen(true);
              }}
              className="block w-full text-center bg-[#00a2ff] hover:bg-[#008ce0] text-white font-black py-3.5 sm:py-4 rounded-xl uppercase tracking-wider text-base sm:text-lg shadow-lg transition active:scale-95 cursor-pointer outline-none border-none"
            >
              PLAY ADOPT ME NOW
            </button>
          </div>
